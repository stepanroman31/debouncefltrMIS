    ...
    if (cntPerformCom++ >= 50 ) {
        ...
        switch (activeCmd) {
            ...
            case 1: // CMD(1): Potenciometr do grafu (1x int16_t)
            {
                ...
                // Tím vy?e?íte výtku, ?e signalizace nefunguje správn?.
            ...
            case 3: // CMD(3): Do Table Terminalu
            {
                ...
                if (aktualniStav == PLC_PROG) {
                    // V re?imu PROG chceme vid?t, kolikátý krok zrovna definujeme.
                    // To odpovídá aktuální délce sekvence (nap?. mám ulo?eny 2 kroky, te? d?lám 3. krok = index 2).
                    idx = getPlcSequenceLength(); 
                } else {
                    // V re?imu RUN/TEST/STOP chceme vid?t, který krok se práv? vykonává.
